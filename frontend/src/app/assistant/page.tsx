...
export default function AssistantPage() {
  const [messages, setMessages] = useState<{role: 'user'|'assistant', text: string}[]>([
    { role: 'assistant', text: 'I am your offline emergency assistant. Please specify your emergency: flood, earthquake, or fire.' }
  ]);
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <h2 className="text-3xl font-bold text-white mb-2">Emergency Assistant</h2>
      <p className="text-slate-400 mb-4">Offline rule-based triage system powered by Python AI Microservice.</p>
        ...
        <div className="flex-1 overflow-y-auto space-y-4">
          {messages.map((m, i) => (
            <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`p-3 rounded-2xl max-w-lg ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-700 text-slate-200 rounded-bl-none'}`}>
                {m.text}
              </div>
            </div>
          ))}
      ...
      <div className="flex gap-2">
        <input 
          ...
          placeholder="Type your emergency (e.g., flood, earthquake, fire)..." 
